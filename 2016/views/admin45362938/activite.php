<h1>Dernière connexion au site pour chaque équipe</h1>
...
$search = $bdd->prepare('SELECT t.equipe, t.date FROM rallye_log t INNER JOIN (SELECT equipe, max(date) AS MaxDate FROM rallye_log GROUP BY equipe) tm ON t.equipe = tm.equipe AND t.date = tm.MaxDate WHERE t.equipe != "terpsichore" AND t.equipe != "euterpe" ORDER BY t.date DESC');
...
if ($search->execute()) {
	while ($team = $search->fetch()) {
		...
		echo '<div class="infoItem" style="opacity:'.$opa.';"><b>'.$team['equipe'].'</b> (il y a '.$old.')</div>';
	}
}
