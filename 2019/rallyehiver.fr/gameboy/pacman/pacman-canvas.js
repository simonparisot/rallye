...
/* ----- Global Variables ---------------------------------------- */
	...
	function Game() {
		...
		this.nextLevel = function() {
			...
			game.showMessage("Niveau "+game.level, this.getLevelTitle() + "<br/>(Cliquez pour continuer !)");
		...
		this.pauseResume = function () {
			if (!this.running) {
				...
				this.showMessage("Pause","Cliquez pour reprendre");
				}
			};
	...
	function pacman() {
		...
		this.dieFinal = function() {
	    	...
	    	if (this.lives <= 0) {
				...
				var audioTheme = document.getElementById("theme");
				audioTheme.pause();
				var input = "<div id='highscore-form'><span id='form-validater'></span><input type='text' id='playerName'/><span class='button' id='score-submit'>Enregistrer</span></div>";
				game.showMessage("Game over","Score total : "+game.score.score);
				game.gameOver = true;
				//$('#playerName').focus();
			}
